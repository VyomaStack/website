...
interface AiResultPanelProps {
  ...
  description: string;
  buttonLabel: string;
  onGenerate: () => Promise<AiGenerateResult>;
  disabled?: boolean;
}
...
export function AiResultPanel({
  ...
  async function handleGenerate() {
    ...
    setError(null);
    try {
      const { text, source: resSource } = await onGenerate();
      setResult(text);
      setSource(resSource ?? "instant");
    } catch (e) {
...
interface AiCodePanelProps {
  ...
  onGenerate: (type: string, className?: string) => Promise<AiGenerateResult>;
  disabled?: boolean;
...
export function AiCodePanel({
  ...
  async function handleGenerate() {
    ...
    setError(null);
    try {
      const { text, source: resSource } = await onGenerate(type, className);
      setResult(text);
      setSource(resSource ?? "instant");
    } catch (e) {
      const message = e instanceof Error ? e.message : "Something went wrong";
